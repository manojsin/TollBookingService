...
public class FourWheelerBookingServiceImp implements BookingService {
    ...
    @Autowired
    public FourWheelerBookingServiceImp(BookingDetailRepository bookingDetailRepository,
    ...
    public CheckValidityResponse checkValidity(String registrationNumber) {
        ...
        response.setStatus(responseUtil.populateBasicResponse(TollServiceStatus.USER_VALID_PASS));
        return response ;
    }
    @Override
    public PassDetailResponse getAllPassDetails(String vehicleType,Long tollId) {
        ...
        if(!tollDetail.isPresent()) {
            response.setStatus(responseUtil.populateBasicResponse(TollServiceStatus.PASS_NOT_AVAILABLE_FOR_FOUR_WHEELER));
            return response;
        }
        ...
        success.setStatus(responseUtil.populateBasicResponse(TollServiceStatus.PASS_AVAILABLE));
        return success;
    }
    ...
    public PassBookingResponse issuePass(PassBookingRequest passBookingRequest) {
        ...
        if(!bookingDetail1.isPresent()) {
            response.setStatus(responseUtil.populateBasicResponse(TollServiceStatus.PASS_BOOKING_FAILED));
            return response;
        }
        ...
        response.setStatus(responseUtil.populateBasicResponse(TollServiceStatus.PASS_BOOKING_SUCCESS));
        return response;
    }
}
